...
def pluslongmot(tirage):
    ...
    for m in mots:
        ...
        maximum=max(points)
    return mots[-1], maximum, mots[points.index(maximum)] #la fonction retourne le mot le plus long , le maximum de points et le mot associé

#EXO 3

valeur={'a':1,'e':1,'i':1,'l':1,'n':1,'o':1,'r':1,'s':1,'t':1,'u':1,'d':2,'g':2,'m':2,'b':3,'c':3,'p':3,'f':4,'h':4,'v':4,'j':8,'q':8,'k':10,'w':10,'x':10,'y':10,'z':10}
def score(mot):
    s=0
    for x in mot:
        s=s+valeur[x]
